  ...
  <div class="container">
    ...
    <div class="flex1" v-show="!search">
      ...
      <div>
        ...
        <ul v-for="(item,index) in fuck" :key="index" style="color:#fff">
          <li class="item">
            {{item.name}}
          </li>
        </ul>
...
export default {
  ...
  mounted() {
    this.contacts = this.contacts.map((contact) => {
      let pin = pinyin(contact.name, { filterChinese: true })
      //var max = !/^[^a-zA-Z]/g.test(pin) ? pin.substring(0, 1).toUpperCase() : String.fromCharCode(35);
      //console.log(max)
      if (!/^[^a-zA-Z]/g.test(pin)) {
        return {
          pin,
        ...
        this.fuck.push({ name: contact.name, pin: pin })
  ...
  watch: {
    search(newS) {
      ...
      if (!newS) return
      if (!this.searchContacts.length) {
        this.searchContacts = this.contacts.filter((c, i) => {
          return c.name.toLowerCase().indexOf(newS) >= 0 || c.pin.toLowerCase().indexOf(newS) >= 0
        })
      } else {
        this.searchContacts = this.fuck.filter((c, i) => {
          return c.name.toLowerCase().indexOf(newS) >= 0 || c.pin.toLowerCase().indexOf(newS) >= 0
        })
      }
