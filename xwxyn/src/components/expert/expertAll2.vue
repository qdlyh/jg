<template>
    <div>
        <tab>
            <tab-item @on-item-click="onItemClick(index)" :selected="index==2" v-for="(item,index) in tab" :key="index">{{item.tab}}</tab-item>
        </tab>
        <div id="mescroll" class="mescroll">
            <div id="dataList" class="data-list" v-cloak>
                <div class="questions-history" v-for="(item,index) in list" :key="item.uuid">
                    <h1>{{item.title}}</h1>
    ...
    methods: {
        ...
        getListDataFromNet(pageNum, pageSize, successCallback, errorCallback) {
            setTimeout(() => {
                this.$ajax({
                    method: 'get',
                    url: this.psta + '/getWxConsultingList?wxUserId=' + this.$parent.wxUserId + '&page=' + pageNum + '&size=' + pageSize,
                })
                    .then(response => {
