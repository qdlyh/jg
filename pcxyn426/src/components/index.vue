...
export default {
  ...
  methods: {
    ...
    tabBtn(tab, item) {
      localStorage.setItem('typeUrl', 'getPcNewsByUuid?'); //根据文章传过来的请求内容地址
      localStorage.setItem('leftNavUrl', 'getPcPageNews?');   //根据文章传过来的请求左侧菜单栏地址
      localStorage.setItem('labelTitle', item.title);    /*article 侧边栏文章列表标题 */
      localStorage.setItem('articleId', item.uuid);   /*article 侧边栏文章列表请求id */
      localStorage.setItem('titleId', item.uuid);   /* 存储分页内容标签id左侧菜单 */
      localStorage.setItem('parentId_websiteId', item.websiteId);  /* 存储分页内容标签id主页内容 */
      this.$router.push('/article');
      //console.log(tab)
      console.log(item)
